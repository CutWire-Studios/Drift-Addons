...
| Essential Transitions | Fades, wipes, slides and other shot-to-shot transitions, including ports of the [GL Transitions](https://gl-transitions.com/) collection |
...
**[GL Transitions](https://gl-transitions.com/)** — most of the transitions in *Essential
Transitions* are ports of the GL Transitions collection, an open catalogue of GPU transition
shaders started by [Gaëtan Renaudeau](https://github.com/gre). Over fifty named authors appear
across the transitions ported here. It is MIT-licensed, and the shader bodies in this repository
are theirs, not ours.

- Website and gallery: <https://gl-transitions.com/> — the [gallery](https://gl-transitions.com/gallery)
  previews every transition, and the [editor](https://gl-transitions.com/editor) runs them live in
  the browser
- Source: <https://github.com/gl-transitions/gl-transitions>

Ports are generated by [`recipes/import-gl-transitions.py`](recipes/import-gl-transitions.py) from
a pinned upstream commit rather than hand-copied, so a re-run reproduces them and an upstream fix
can be pulled in. Each generated `main.frag` keeps its original `Author:` / `License:` header, and
[`content/transitions/ATTRIBUTION.md`](content/transitions/ATTRIBUTION.md) indexes every port with
the upstream file and its author. If you are looking for who wrote a particular transition, that
table is the place.

Please credit the original authors if you reuse these shaders, and consider contributing
improvements back upstream rather than only here.
...
*Essential Transitions* is `GPL-3.0 AND MIT` for this reason: the transitions written for Drift are
GPL-3.0, while the GL Transitions ports stay MIT under their original authors — see
[Credits](#credits). MIT is compatible with GPL-3.0, so the pack redistributes cleanly, but the
MIT notice travels with the shaders: it is in every ported `main.frag` and in
`content/transitions/ATTRIBUTION.md`, both of which ship inside the package.
